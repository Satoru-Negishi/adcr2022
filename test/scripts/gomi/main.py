...
class main_infomation(object):
    ...
    def key_reception(self):
        ...
        while True:
            ...
            for word in l:
                if not word in ['head','waist','arm']:
                    Inapplicable_characters = True
            if Inapplicable_characters == False:
                break
        rospy.loginfo("motor used: %s", motor_sel)
    ...
    def pose_messageCb(self,msg,motor_sel_l):
        ...
        if (('ALL' in motor_sel_l) or (len(motor_sel_l) == 3)):
            self._mian_pose_pub(msg)
        else:
            unused = list(set(['head,waist,arm'])^ set(motor_sel_l))
            nparray_msg=_multiarray2numpy(uint16, np.uint16, msg)
            for site in unused:
                if site == 'head':
                    nparray_msg[0] = 90
                elif site == 'waist':
    ...
    def blink_messageCb(self,msg,blink_detc):
        ...
        if blink_detc == 'yes':
            ...
            self._main_blink_pub(self.blink)
        else:
            self.blink.data =random.choice(['no_blink','blink','blink','blink'])  
            self._main_blink_pub(self.blink)
    ...
    def emo_messageCb(self,msg,exp_fil):
        ...
        if exp_fil == 'yes': #フィルタあり
            ...
            self._main_emotion_pub(self.emotion)  
        else:
            self.emotion.data = msg.data
            self._main_emotion_pub(self.emotion)       
